...
class IpswichEkidenTeamDeclarationAPIControllerV1
{
  ...
  public function permission_check(\WP_REST_Request $request)
  {
    ...
    if (!($current_user instanceof \WP_User) || $current_user->ID == 0) {
      return new \WP_Error(
        ...
        array('status' => 403, 'User' => $current_user->ID)
      );
    }
  ...
  public function create_user(\WP_REST_Request $request)
  {
    ...
    if (is_wp_error($user_id)) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('Registration failed for username %s', $email),
        array('status' => 400)
      );
    }
  ...
  public function update_team(\WP_REST_Request $request)
  {
    if (!$this->is_valid_team_captain($request['id'])) {
      return new \WP_Error(
        'rest_forbidden',
        sprintf('You do not have enough privileges to update this team.'),
        array('status' => 403)
      );
    }
  ...
  public function delete_team(\WP_REST_Request $request)
  {
    if (!$this->is_valid_team_captain($request['id'])) {
      return new \WP_Error(
        'rest_forbidden',
        sprintf('You do not have enough privileges to delete this team.'),
        array('status' => 403)
      );
    }
  ...
  public function add_team_runner(\WP_REST_Request $request)
  {
    if (!$this->is_valid_team_captain($request['id'])) {
      return new \WP_Error(
        'rest_forbidden',
        sprintf('You do not have enough privileges to use this update this team.'),
        array('status' => 403)
      );
    }
  ...
  public function update_team_runner(\WP_REST_Request $request)
  {
    ...
    if (! $team || empty($team->runners)) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('Team %d does not exist or has no runners.', $request['id']),
        array('status' => 404)
      );
    }
    ...
    if (! $runner) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('Runner leg %d not found for team %d.', $request['leg'], $request['id']),
        array('status' => 404)
      );
    }
    ...
    switch ($field) {
      ...
      default:
        return new \WP_Error(
          'rest_invalid_param',
          sprintf('The field %s is not allowed.', $field),
          array('status' => 400)
        );
    }
  ...
  public function is_valid_runner_leg($value, $request, $key)
  {
    if (!in_array($value, array(1, 2, 3, 4, 5, 6))) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s %d must between 1 and 6', $key, $value),
        array('status' => 400)
      );
    } else {
      return true;
  ...
  public function is_valid_team_update_field($value, $request, $key)
  {
    if ($value == 'name' || $value == 'clubId') {
      ...
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s %d must be name or clubId only.', $key, $value),
        array('status' => 400)
      );
    }
  }
  ...
  public function is_valid_new_user($model, $request, $key)
  {
    if (empty($model['email'])) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s has invalid email value.', $key),
        array('status' => 400)
      );
    }
    ...
    if (empty($model['password'])) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s has invalid password value.', $key),
        array('status' => 400)
      );
    }
    ...
    if (empty($model['firstName'])) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s has invalid firstName value.', $key),
        array('status' => 400)
      );
    }
    ...
    if (empty($model['lastName'])) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s has invalid lastName value.', $key),
        array('status' => 400)
      );
    }
  ...
  public function is_valid_email($value, $request, $key)
  {
    if (! is_email($value)) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s is not a valid email address.', $key),
        array('status' => 400)
      );
    }
  ...
  public function is_valid_password($value, $request, $key)
  {
    if (strlen(trim($value)) < 8) {
      return new \WP_Error(
        'rest_invalid_param',
        sprintf('%s must be at least 8 characters.', $key),
        array('status' => 400)
      );
    }
