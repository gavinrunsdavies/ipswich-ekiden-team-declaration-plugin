...
class IpswichEkidenTeamDeclarationDataAccess
{
  ...
  public function add_team_runner($teamId, $leg, $name, $gender, $ageCategory, $dateOfBirth = null, $medicalInfo = null)
  {
    ...
    if ($result) {
      ...
      if (!$result) {
        return new \WP_Error(
          'add_team_runner',
          'Unknown error in updating team in to the database',
          array('status' => 500)
        );
      }
    }
  ...
  public function delete_team($id)
  {
    ...
    if (!$result) {
      return new \WP_Error(
        'delete_team',
        'Unknown error in deleting team from the database',
        array('status' => 500)
      );
    }
  }
  ...
  public function get_club_team_count()
  {
    ...
    if (!$result) {
      return new \WP_Error(
        'get_club_team_count',
        'Unknown error in getting club team statistics from the database',
        array('status' => 500)
      );
    }
  ...
  public function get_runner_category_count()
  {
    ...
    if (!$result) {
      return new \WP_Error(
        'get_runner_category_count',
        'Unknown error in getting club runner statistics from the database',
        array('status' => 500)
      );
    }
